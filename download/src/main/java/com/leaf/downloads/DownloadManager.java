...
public class DownloadManager {
    ...
    public final static String ACTION_DOWNLOAD_COMPLETE = "com.leaf.downloads.action.DOWNLOAD_COMPLETE";

    public final static String ACTION_NOTIFICATION_CLICKED =
            "com.leaf.downloads.action.DOWNLOAD_NOTIFICATION_CLICKED";

    public final static String ACTION_VIEW_DOWNLOADS = "com.leaf.downloads.action.VIEW_DOWNLOADS";
    ...
    public static class Request {
        ...
        ContentValues toContentValues(String packageName) {
            ...
            values.put(Downloads.Impl.COLUMN_ICON_URI, mIconUrl);
    ...
    public void removeAll() {
        String order = Downloads.Impl._ID + " desc";
        Cursor cursor = mResolver.query(mBaseUri, null, null, null, order);
    ...
    public int getDownloadStatus(long downloadId) {
        String selection = Downloads.Impl._ID + " = " + downloadId;
        String orderBy = Downloads.Impl._ID + " DESC";
        Cursor cursor = mResolver.query(Downloads.Impl.CONTENT_URI, null, selection, null, orderBy);
        if (cursor != null && cursor.getCount() > 0) {
            cursor.moveToFirst();
            return cursor.getInt(cursor.getColumnIndexOrThrow(Downloads.Impl.COLUMN_STATUS));
        }
        return -1;
    }

    public int getDownloadId(String uri) {
        String selection = Downloads.Impl.COLUMN_URI + " = " + uri;
        String orderBy = Downloads.Impl._ID + " DESC";
        Cursor c = mResolver.query(Downloads.Impl.CONTENT_URI, null, selection, null, orderBy);
        if (c != null && c.getCount() > 0) {
            c.moveToFirst();
            return c.getInt(c.getColumnIndex(Downloads.Impl._ID));
        }
        return -1;
    }
    ...
    public void restartDownload(long... ids) {
        ...
        try {
            for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
                ...
                int idColumnId = cursor.getColumnIndex(COLUMN_ID);
                if (status != STATUS_SUCCESSFUL && status != STATUS_FAILED) {
//                    throw new IllegalArgumentException("Cannot restart incomplete download: "
//                            + cursor.getLong(cursor.getColumnIndex(COLUMN_ID)));
                    cancelDownload(cursor.getLong(idColumnId));
                }
              /*  String filename = cursor.getString(filepathId);
    ...
    public File getDownloadDirectory() {
        File file = mApplicationContext.getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS);
    ...
    public int pauseDownload(long... ids) {
        ...
        if (ids.length == 1) {
            ...
            return mResolver.update(mBaseUri, values, selection, getWhereArgsForIds(ids));
